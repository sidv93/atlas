...
@Component({
  selector: 'app-progression-test',
  templateUrl: './progression-test.component.html',
  styleUrls: ['./progression-test.component.css']
})
export class ProgressionTestComponent implements OnInit {
  ...
  public getProgressionData() {
    this.loading = true;
    this.optimiseService.getProgressionData('Unigy', 'U_C_P_2.0', 'Callprocessing').subscribe(
      data => {
        ...
        let backgroundColor = this.getBackgroundColor(this.graphData);
        const dataset = [
          {
            data: graphData,
        ...
        this.progressionChart = this.globalService.makeProgressionGraph('progressionChart', graphLabels, dataset);
        this.loading = false;
      },
      error => {
  ...
  public graphPriorityFilter() {
    ...
    this.progressionChart.data.datasets[0].data = graphData;
    this.progressionChart.data.labels = graphLabels;
    this.progressionChart.update();
  ...
  public rangeChange(value: any) {
    ...
    console.log('new test0-' + JSON.stringify(newGraph));
